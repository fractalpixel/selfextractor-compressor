...
export type RecursiveReplacerConfig = {

    /**
     * Number of rounds to calculate.
     * Best result is picked.
     * If parameterVariation is zero and topReplacementsToSelectFrom is 1, then only one round is done.
     */
     rounds: number,

     /**
     * If larger than zero, try different combination of parameters, toggling flags on and off and tuning numerical parameters up and down.
     * 
     * When sweeping parameter combinations, how different values should be tested compared to the ones 
     * specified in the config?
     * 0 = no variation, use config parameters, same as multiple rounds.
     * 1 = max variation, disregard parameters in the config, pick random parameter values from allowed range.
     * 0.5 = pick randomly between random parameter values and config parameter values, weighted by this parameter.
     */
    parameterVariation: number,

    /**
     * Number of best repalcements to select from each round.
     * If 1, selects only the most optimal, but that might not result in the best total compression.
     */
    topReplacementsToSelectFrom: number

    /**
     * 1 is more likely to select best scoring replacement
     * at each step (which is not necessarily ideal for the whole file).
     * 0 selects any of the top [substringsToSelectFrom] replacements with
     * equal probability.
     * Any number in between can be used.
     */
    selectionFocus: number,

    /**
     * How many unused characters are reserved for single character replaces vs. multi character replaces.
     * Multi characters allow for more total replacements, but take more space.
     * Single character replaces are useful for compacting very frequent strings compactly (e.g. "this.").
     */
    fractionOfSingleCharacterKeys: number,

    /**
     * For two-character keys, uses a base-36 character (small letters + digits),
     * This increases number of possible replacements, especially if there are few unused characters 
     * to use as replacement id start characters, but adds about 15 bytes compared to just using digits.
     * Set to true for larger files (about 4k+ ?)
     */
    useAlphanumericMultiCharacterKeys: boolean,

    /**
     * String to use as random seed when trying out different replacements.
     * The same source, seed, config settings, and compressor version produces the same result.
     */
    randomSeed: string,

    /**
     * Output progress information and parameter settings
     */
    verbose: boolean,
}

export default async function compress(source: string, logger: Logger<ILogObj>, config: RecursiveReplacerConfig = DEFAULT_CONFIG): Promise<string> {
    ...
    let alternativeResults: ConfigResult[] = [{ config: config, result: source }]
    
    function getNthFractionBestResult(n: number = 0): ConfigResult {
    ...
    function stats(result: string): string {
    ...
    for (let round = 1; round <= config.rounds; round++) {
        ...
        let baseConfig: RecursiveReplacerConfig = utils.shallowCopyObj(getNthFractionBestResult(configFocus).config)
        ...
        if (config.verbose) {
            ...
            logger.info(
                `Round ${round.toString().padStart(config.rounds.toString().length)} / ${config.rounds} (${Math.round(100 * round / config.rounds).toString().padStart(3)}%): ${improvementIcon} ${stats(result)}. Best is ${bestStats}.` +
    ...
    if (result.length >= source.length) {
        result = source
        if (config.verbose) logger.debug("Could not find an efficient compression, returning original source")
    }
    else {
        if (config.verbose) {
...
function tuneConfig(config: RecursiveReplacerConfig, round: number, rand: RandomSeed): RecursiveReplacerConfig {
    function randomValue(base: number, min: number = 0, max: number = 1): number {
    ...
    function randomBoolean(base: boolean): boolean {
    ...
    let roundConfig: RecursiveReplacerConfig = {
 ...
 */
 async function encode(input: string, config: RecursiveReplacerConfig, round: number, logger: Logger<ILogObj>): Promise<string> {
    ...
    let bestResult: SubstitutionResult = {substitutions: new Map(), value: 0, sourceWithReplacements: input}
...
type SubstitutionResult = {substitutions: Map<string,utils.Replacement>, value: number, sourceWithReplacements: string }

function findSubstitutions(source: string, substitutionCharacters: string[], singleKeyCharCount: number, random: Random.RandomSeed, substringsToSelectFrom: number, config: RecursiveReplacerConfig): SubstitutionResult {
    ...
    let replacements: Map<string,utils.Replacement> = new Map()
    let substitutionCount = 0

    let id: string | undefined = undefined
    let selectedReplacement: utils.Replacement | undefined = undefined
...
function claculateKey(i: number, substitutionCharacters: string[], singleKeyCharCount: number, config: RecursiveReplacerConfig): string | undefined {
    if (substitutionCharacters.length <= 0) return undefined // No unused characters

    // Rest can be followed by one other id character
    let multiKeyCount = substitutionCharacters.length - singleKeyCharCount

    if (i < singleKeyCharCount) return substitutionCharacters[i]
    else {
...
function buildUnpacker(replacements: Map<string,utils.Replacement>, encodedSource: string, separator: string, keyCharacters: string[], singleKeyCount: number, config: RecursiveReplacerConfig): string {
